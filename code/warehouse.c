 ...
 *
 * Usage:
 *   ./warehouse <num_receivers> <num_pickers> <num_packers> <queue_capacity> <inventory.csv>
 *
...
/* ---- Thread-argument structs: passed by reference, owned by main ---- */
typedef struct {
...
/* Atomic stock decrement. Grabs inv->mutex itself.
 * Returns units shipped and sets *err. */
static int inventory_decrement(Inventory *inv, int item_id, int qty, int *err)
{
    int ship = 0;
    ...
    if (!it) {
        ...
        *err = ERR_OUT_OF_STOCK;
    } else {
        ship = (it->stock >= qty) ? qty : it->stock;
        it->stock -= ship;
...
/* Stock increment, grabs inv->mutex itself (small CS).
 * Returns new stock, or -1 if the item does not exist. */
...
static int bq_produce(BoundedQueue *q, const Order *o)
{
    ...
    while (q->count == q->capacity && !q->shutdown)
        pthread_cond_wait(&q->not_full, &q->mutex);     /* If full, it releases the mutex and waits for not_full */
    if (q->count == q->capacity && q->shutdown) {       /* We're done waiting: if we can produce we do, if we are full and in shutdown we return -1*/
        pthread_cond_signal(&q->not_full);              /* a chain reaction among other producers */
...
static int bq_consume(BoundedQueue *q, Order *out)
{
    ...
    while (q->count == 0 && !q->shutdown)
        pthread_cond_wait(&q->not_empty, &q->mutex); /*if empty release the mutex and wait for not_empty*/
    if (q->count == 0) { /*we are in shutdown*/
...
static void log_order(int log_fd, pthread_mutex_t *log_mutex, const Order *o)
{
    ...
    char line[LINE_BUF];
    int n = snprintf(line, sizeof(line), "%s|%d|%s|%d|%d|%d|%d|%s\n", tbuf, o->order_id, o->request.client_id, o->request.item_id,o->request.quantity, o->qty_shipped, o->qty_rejected, st);
    if (n <= 0) return;
...
static void send_response(const char *resp_fifo, const OrderResponse *resp)
{
    ...
    if (fd < 0) {
        fprintf(stderr, "[WAREHOUSE] undeliverable response for '%s': %s (client terminated?)\n", resp_fifo, strerror(errno));
        return;
    }
...
/* Sleep a random 1-3 s to simulate physical work. */
static void rand_sleep_1_3(void)
{
    sleep(1 + rand() % 3);
}

/* ====== Signal handlers: set a flag only ======  */
...
static void write_status_file(Inventory *inv, BoundedQueue *pending, BoundedQueue *packaging, int n_rec, int n_pick, int n_pack)
{
    ...
    if (snap) {
        for (int i = 0; i < count; i++) {
            n = snprintf(buf, sizeof(buf), "ITEM|%d|%s|%s|%d\n",snap[i].item_id, snap[i].description, snap[i].category, snap[i].stock);
            if (n > 0) write_all(fd, buf, (size_t)n);
        }
        free(snap);
    }
 ...
 * ========================================================================== */
static void log_rejection(int log_fd, pthread_mutex_t *log_mutex, const OrderRequest *req, int order_id, int err)
{
    Order o;
    memset(&o, 0, sizeof(o));
    o.request = *req;
...
static void *receiver_thread(void *arg)
{
    ...
    while (!shutdown_flag) {
        ...
        pthread_mutex_lock(a->order_id_mutex);
        int order_id = (*a->next_order_id)++;
        pthread_mutex_unlock(a->order_id_mutex);
...
static void *picker_thread(void *arg)
{
    ...
    for (;;) {
        ...
        int err;
        int shipped = inventory_decrement(a->inv, o.request.item_id,o.request.quantity, &err);
        ...
        if (shipped == 0) {        /* ran out of item between validation and pick */
            o.status = ORDER_REJECTED;
            OrderResponse resp = { err, o.request.item_id, o.request.quantity, 0, o.request.quantity };
...
static void *packer_thread(void *arg)
{
    ...
    for (;;) {
        ...
        OrderResponse resp = { status, o.request.item_id, o.request.quantity, o.qty_shipped, o.qty_rejected };
        send_response(o.request.resp_fifo, &resp);
        log_order(a->log_fd, a->log_mutex, &o);
    }
    return NULL;
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 6) {
        fprintf(stderr, "Usage: %s <num_receivers> <num_pickers> <num_packers> <queue_capacity> <inventory.csv>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if (num_receivers <= 0 || num_pickers <= 0 || num_packers   <= 0 || queue_cap   <= 0) {
    ...
    if (bq_init(&pending, queue_cap) != ERR_OK || bq_init(&packaging, queue_cap) != ERR_OK) {
    ...
    ReceiverArgs receiver_args = { orders_fd, &orders_read_mutex, &inv, &pending, &next_order_id, &order_id_mutex, log_fd, &log_mutex };
    ...
    while (!shutdown_flag) {
        ...
        if (dump_status_flag) {
            write_status_file(&inv, &pending, &packaging, num_receivers, num_pickers, num_packers);
            dump_status_flag = 0;
        }
    }
