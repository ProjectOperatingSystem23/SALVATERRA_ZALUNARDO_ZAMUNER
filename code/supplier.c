...
static int load_config(const char *path, RestockSchedule *plan, int max_items)
{
    ...
    /* Mandatory header. */
    if (read_line_from_fd(fd, line, sizeof(line)) <= 0 || strncmp(line, CONF_HEADER, strlen(CONF_HEADER)) != 0) {
        fprintf(stderr, "[SUPPLIER] '%s': missing/invalid header (expected '%s')\n", path, CONF_HEADER);
        close(fd);
        return -1;
    }
    ...
    while (count < max_items && (n = read_line_from_fd(fd, line, sizeof(line))) > 0) {
        ...
        if (sscanf(line, "%d,%d,%d", &it->item_id, &it->quantity, &it->interval) != 3) {
            fprintf(stderr, "[SUPPLIER] '%s': malformed line: %s", path, line);
            close(fd);
            return -1;
        }
        if (it->item_id < 1 || it->quantity < 1 || it->interval < 1) {
            fprintf(stderr, "[SUPPLIER] '%s': non-positive values: %s", path, line);
            close(fd);
            return -1;
        }
...
int main(int argc, char *argv[])
{
    ...
    while (!shutdown_flag) {
        ...
        /* interrupted sleep returns the seconds NOT slept -> slept is the time actually elapsed */
